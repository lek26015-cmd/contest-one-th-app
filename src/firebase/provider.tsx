    ...
    if (!userDoc.exists()) {
        ...
        try {
            await setDoc(userDocRef, {
                ...
                profileName: user.displayName || user.email?.split('@')[0] || 'Anonymous User',
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(
      ...
      async (firebaseUser) => { // Auth state determined
        ...
        if (firebaseUser) {
            // When user logs in, ensure their document exists in Firestore
            createUserDocument(firestore, firebaseUser);
            
            // Sync ID token to cookie for server-side auth
